  ...
  useEffect(() => {
    const fetchTratamientos = async () => {
      try {
        ...
        setTratamientos(data.filter(t => t.categoria && t.categoria.replace(/\s/g, '').toLowerCase() === 'corporal'));
      } catch (err) {
    ...
    <>
      ...
      <BackgroundService>
        ...
        {!loading && !error && tratamientos.length === 0 && (
          <div style={{ textAlign: 'center', padding: '2rem' }}>No hay tratamientos corporales disponibles.</div>
        )}
          ...
          <FadeIn delay={0.2 + idx * 0.1} key={tratamiento.id}>
            {idx % 2 === 0 ? (
              <TreatmentsLeft
                ...
                detailsLink={"/servicios/corporales/" + tratamiento.id}
